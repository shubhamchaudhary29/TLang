...
Client requests are synchronous. Server handlers share the interpreter lock, so handlers execute serially even when the underlying HTTP server accepts multiple connections.
